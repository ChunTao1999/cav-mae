...
exp_name = f"ResNet18_{args.start_lr}_{args.num_epochs}_{args.exp_scheduler_gamma}_{loss_ratio[0]}_{loss_ratio[1]}_{loss_ratio[2]}_{loss_bbox_ratio[0]}_{loss_bbox_ratio[1]}_{loss_bbox_ratio[2]}_{loss_bbox_ratio[3]}"
...
#%% To-dos:
# 1) Deal with classification imbalance (done)
# 2) Freeze layers when finetuning (done)
# 3) Add validation (done)
# 4) Add tensorboard to visualize all loss changes during training (done)